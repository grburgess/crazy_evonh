...
crazy_evonh_dev_log_handler = handlers.TimedRotatingFileHandler(
...
crazy_evonh_dev_log_handler.setFormatter(_dev_formatter)
crazy_evonh_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

crazy_evonh_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

crazy_evonh_usr_log_handler.setLevel(logging.INFO)
...
crazy_evonh_usr_log_handler.setFormatter(_usr_formatter)
...
crazy_evonh_console_log_handler = logging.StreamHandler(sys.stdout)
crazy_evonh_console_log_handler.setFormatter(_console_formatter)
crazy_evonh_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    crazy_evonh_usr_log_handler.addFilter(warning_filter)
    crazy_evonh_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    crazy_evonh_usr_log_handler.removeFilter(warning_filter)
    crazy_evonh_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    crazy_evonh_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(crazy_evonh_dev_log_handler)

    log.addHandler(crazy_evonh_console_log_handler)

    log.addHandler(crazy_evonh_usr_log_handler)
